...
function start(bus) {
  ...
  const REG = [
    ...
    ['Desks', 'commodity-desks', b => b.commodDesksSummary && `8 desks · ${b.commodDesksSummary.filter(s => s.stance !== 'NEUTRAL').length} active · ${b.commodDesksSummary.filter(s => s.stance === 'BULLISH').map(s => s.emoji).join('')}`],
  ...
  function age(b, name) {
    ...
    const DESC = {
      ...
      'commodity-desks': '8 thematic commodity desks (energy/metals/agri/softs/industrial/battery/index/vol) → advisory signals',
