...
function build() {
  ...
  for (const [region, cfg] of Object.entries(REGIONS)) {
    for (const dom of cfg.outlets) {
      ...
      add(gn(`site:${dom} when:2d`, cfg.hl, cfg.gl), cap(label), region);
      add(gn(`site:${dom} (stocks OR markets OR economy) when:2d`, cfg.hl, cfg.gl), cap(label), region);
      add(gn(`site:${dom} (analysis OR forecast OR outlook OR earnings) when:2d`, cfg.hl, cfg.gl), cap(label), region);
    }
