...
const DIRECT = [
  ...
  // YouTube RSS (free, keyless) — finance-news channels; relevance filter keeps market clips
  ['https://www.youtube.com/feeds/videos.xml?channel_id=UCIALMKvObZNtJ6AmdCLP7Lg', 'YouTube·Bloomberg', 'US'],
  ['https://www.youtube.com/feeds/videos.xml?channel_id=UCvJJ_dt2ItYptdy5ef4Gm6A', 'YouTube·CNBC', 'US'],
  ['https://www.youtube.com/feeds/videos.xml?channel_id=UCEAZeUIeJs0IjQiqTCdVSIg', 'YouTube·YahooFin', 'US'],
  ['https://www.youtube.com/feeds/videos.xml?channel_id=UCrp_UI8XtuYfpiqluWLD7Lw', 'YouTube·CNBCtv', 'US'],
];
...
const CRYPTO_TOPICS = ['bitcoin price', 'ethereum', 'solana', 'crypto regulation', 'crypto ETF', 'stablecoin', 'DeFi', 'crypto hack', 'bitcoin ETF flows', 'altcoin season', 'XRP ripple', 'crypto SEC', 'coinbase OR binance', 'crypto market crash OR rally', 'memecoin', 'ethereum ETF', 'crypto whale', 'bitcoin halving OR miners', 'crypto adoption'];

// Everything Trump — a dense dedicated lane spanning his posts (Truth Social), his social
// activity as reported (X/Twitter, Reddit, Telegram, YouTube), news-channel coverage, the
// bills/executive orders he signs, and what he/his family own & trade.
const TRUMP_QUERIES = [
  ...
  // his social activity as reported across platforms
  'Trump "on X" OR "on Truth Social" post', 'Trump tweet OR "posted on"', 'Trump Reddit OR Telegram',
  'Trump YouTube OR interview OR rally speech', 'Trump reacts OR slams OR praises',
  // bills, orders & policy that move specific sectors
  'Trump signs bill OR executive order', 'Trump quantum computing OR quantum', 'Trump AI executive order OR chips',
  'Trump semiconductor OR CHIPS act', 'Trump defense OR military spending bill', 'Trump drilling OR energy order',
  'Trump immigration OR border policy', 'Trump crypto bill OR strategic reserve', 'Congress bill markets impact',
];

// COMMODITY lane — a dense dedicated feed for the eight commodity desks. Each query is a
// live Google-News channel localised to the market that moves that commodity, plus the
// macro drivers (dollar/DXY, real yields, China demand, weather/crops, OPEC, mine strikes).
const COMMODITY_QUERIES = [
  // energy
  'oil price OR crude oil', 'brent crude OR WTI', 'OPEC OR OPEC+ output', 'natural gas price', 'US shale OR rig count', 'gasoline OR diesel demand', 'LNG export',
  // precious metals
  'gold price', 'silver price', 'platinum OR palladium', 'central bank gold buying', 'safe haven demand',
  // industrial metals
  'copper price', 'aluminium OR aluminum price', 'nickel OR zinc OR tin', 'iron ore price', 'LME metals',
  // agriculture & softs
  'wheat OR corn OR soybean price', 'coffee OR cocoa OR sugar price', 'crop report OR USDA WASDE', 'drought OR harvest OR weather crop', 'grain export OR Black Sea',
  // battery / rare earth / nuclear
  'lithium price OR battery metals', 'rare earth OR cobalt', 'uranium price OR nuclear fuel',
  // macro drivers
  'dollar index OR DXY commodities', 'commodities outlook OR super cycle', 'China commodity demand', 'mine strike OR supply disruption',
];
...
function build() {
  ...
  // Commodity lane — feeds the eight commodity desks
  for (const t of COMMODITY_QUERIES) add(gn(`${t} when:1d`, 'en-US', 'US'), 'GNews·commodity', 'COMMODITY');
