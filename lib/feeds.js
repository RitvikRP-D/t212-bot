...
function parseItems(xml, limit) {
  ...
  for (const b of blocks) {
    ...
    // publish time — without it sentiment age-decay was a silent no-op (every stale
    // item RSS keeps re-serving scored as if fresh)
    const dm = b.match(/<(pubDate|published|updated|dc:date)[^>]*>(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?<\/\1>/i);
    const pubTime = dm && dm[2] ? (Date.parse(decode(stripTags(dm[2]))) || undefined) : undefined;
    if (t && t.length > 4) out.push({ title: t, url, pubTime });
...
async function fetchHeadlines(url, source, limit = 15) {
  ...
  try {
    ...
    return parseItems(xml, limit).map(it => ({ title: it.title, url: it.url || '', pubTime: it.pubTime, source }));
