...
// #new④: SENTIMENT DECAY — weight headlines by age. Older news matters less for entry.
// Returns decay multiplier: 1-day-old = 1.0, 7-day-old = 0.5, 14-day-old = 0.25
function decayByAge(ageHours) {
  if (ageHours == null) return 1.0;
  return Math.exp(-ageHours / (24 * 7));   // exponential decay with 7-day half-life
}

// aggregate a list of {title, source, ageHours?} into a mean score
// #new④: if ageHours is provided on each headline, apply decay weight (older news counts less)
function scoreAll(heads, useDecay) {
  for (const h of heads) {
    h.score = scoreHeadline(h.title, h.source);
    // #new④: apply decay if enabled and age is known
    if (useDecay && h.ageHours != null) h.score *= decayByAge(h.ageHours);
  }
...
module.exports = { scoreHeadline, scoreAll, amountWeight, SOURCE_W, decayByAge };
